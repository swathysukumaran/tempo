...
export const lookupPlace = asyncHandler(async (req: express.Request, res: express.Response) => {
    const { placeName } = req.body;
    const apiKey = process.env.GOOGLE_API_KEY;

    const searchRes = await axios.get("https://maps.googleapis.com/maps/api/place/findplacefromtext/json", {
        params: { input: placeName, inputtype: "textquery", fields: "place_id", key: apiKey },
    });
    ...
    const detailsRes = await axios.get("https://maps.googleapis.com/maps/api/place/details/json", {
        params: { place_id: placeId, fields: "name,geometry,photos", key: apiKey },
    });
